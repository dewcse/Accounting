    ...
    $('#btnSave').click(function () {
        if (Validate()) {
            ...
            for (var i = 1; i < rows.length; i++) {
                ...
                ledgerChild.Debit = 0;
                ledgerChild.Credit = $(rows[i]).find("td:eq(5) input").val();
                ...
                voucherChild.VoucherType = $('#lblVoucherType').val();
                voucherChild.VoucherName = $('#txtVoucherCode').val();
                voucherChild.CashOfAccountType = $('#ddlReceivedIn').val();
...
function LoadChartOfAccount() {
    $.ajax({
        ...
        success: function (data) {
            var result = JSON.parse(data);
            chartOfAccount = _.filter(result, function (val) { return val.groupId == 2; });
...
function BindCashOfAccount(cashOfAccount) {
    ...
    $('#ddlReceivedInOuter').html("<select id='ddlReceivedIn' class='form-control'></select>");
    ...
    $('#ddlReceivedIn').html(html);
    $("input, textarea, select").not('.nostyle').uniform();
}

function LoadVoucherInfo() {
    $.ajax({
        url: getReceiptVoucherNo,
        type: 'GET',
        success: function (data) {
            var result = JSON.parse(data);
            console.log(result);
...
function CalculateAmount() {
    ...
    if (rows.length > 0) {
        ...
        for (var i = 1; i < rows.length; i++) {
            quantity = parseFloat($(rows[i]).find("td:eq(3) input").val(),2);
            unitPrice = parseFloat($(rows[i]).find("td:eq(4) input").val(),2);
            if (isNaN(quantity)) {
                quantity = 0; }
            if (isNaN(unitPrice)) {
                unitPrice = 0; }
            ...
            if (calAmount == 0) {
                amount = parseFloat($(rows[i]).find("td:eq(5) input").val(),2);
                if (isNaN(amount)) {
                    amount = 0;
                } else {
                    amount = amount;
...
function Validate() {
    ...
    if ($('#ddlReceivedIn').val() == -1) {
        toastr.options.timeOut = 3500;
        toastr.warning("Cash Account selection for Received In is required.", "Incomplete");
        return false;
    }
...
function ClearAlfterSave() {
    ...
    //$('#ddlReceivedInOuter').empty();
    //$('#ddlReceivedInOuter').html('<select id="ddlReceivedIn"><option value="-1">Select Cash of Account</option></select>');
    //$("input, textarea, select").not('.nostyle').uniform();
